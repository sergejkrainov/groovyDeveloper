...
class TaskModel implements Serializable {
    ...
    def actionList = new ArrayList<ActionModel>()

    TaskModel(){}

    TaskModel(def title, LocalDate dueDate, LocalTime startTime, LocalTime endTime) {
    ...
    void addAction(def title, LocalTime startTime, LocalTime endTime) {
        this.actionList << new ActionModel(title, startTime, endTime)
    }
    ...
    void editAction(def selectedAction) throws NullPointerException {
        try {
            ...
            ActionModel action = actionList[actionIndex]
    ...
    public void listAllActions(def sortBy) {
        ...
        if (sortBy.equals("2")) {
            ...
            actionList.sort(ActionModel::getTitle)
                    .each {
                        println(String.format(displayFormat,
                                it.getTitle(),
                                it.getStartTime(),
            ...
            actionList.sort(ActionModel::getStartTime)
                    .each {
                        println(String.format(displayFormat,
                                it.getStartTime(),
                                it.getEndTime(),
    ...
    boolean checkActionForInputTimeInterval(LocalTime startTime, LocalTime endTime, ActionModel action) {

        boolean correctTimes = false
        this.actionList
            .sort(ActionModel::getStartTime)
            .each{
                if(!it.equals(action)) {
     ...
     */
    public boolean readActionFromUserToUpdate(ActionModel action) {
        Scanner scan = new Scanner(System.in);
